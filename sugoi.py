...
class JparacrawlTranslator(OfflineTranslator):
    ...
    async def _infer(self, from_lang: str, to_lang: str, queries: List[str]) -> List[str]:
        # === LOGGING OF ORIGINAL TEXT BEFORE TRANSLATION ===
        log_dir = "/content/manga-image-translator/manga_translator"
        os.makedirs(log_dir, exist_ok=True)
        log_path = os.path.join(log_dir, "ocr_log.txt")

        # Добавляем оригинальные строки в лог-файл
